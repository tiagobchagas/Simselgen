...
ggplot(gv_df, aes(x = gv.Trait1, fill = factor(env))) +
  ...
  labs(x = "CS Genetic values for grain yield (t/ha)", y = "Count", fill = "Environment")

ggplot(gv_df, aes(x = gv.Trait2, fill = factor(env))) +
  geom_histogram(color = "#e9ecef", alpha = 0.8, position = "identity", bins = 50) +
  scale_fill_manual(values = c("violetred3", "goldenrod3", "skyblue2")) +
  labs(x = "CS Genetic values for plant heigth (cm)", y = "Count", fill = "Environment")
...
#
ggplot(gv_df_us, aes(x = gv.Trait1, fill = factor(env))) +
  geom_histogram(color = "#e9ecef", alpha = 0.8, position = "identity", bins = 50) +
  scale_fill_manual(values = c("violetred3", "goldenrod3", "skyblue2")) +
  labs(x = "US Genetic values for grain yield (t/ha)", y = "Count", fill = "Environment")

ggplot(gv_df_us, aes(x = gv.Trait2, fill = factor(env))) +
  geom_histogram(color = "#e9ecef", alpha = 0.8, position = "identity", bins = 50) +
  scale_fill_manual(values = c("violetred3", "goldenrod3", "skyblue2")) +
  labs(x = "US Genetic values for plant heigth (cm)", y = "Count", fill = "Environment")
```
...
geno = gv_df |> group_by(env) |> mutate(rank.gy =ifelse(gv.Trait1>mean(gv.Trait1), 1,0)) |>  mutate(rank.ph =ifelse(gv.Trait1>mean(gv.Trait1), 1,0)) |> mutate(rank.sum = rowSums(cbind(rank.ph,rank.gy))) |>  ungroup() 

ind.sel =geno |> group_by(id) |>  summarise(avg=sum(rank.sum)) 

geno = geno |>  pivot_wider(values_from = c(gv.Trait1,gv.Trait2), names_from = c(env) ) |> left_join(ind.sel, by="id") |> as.data.frame()
...
geno_us = gv_df_us |> group_by(env) |> mutate(rank.gy =ifelse(gv.Trait1>mean(gv.Trait1), 1,0)) |>  mutate(rank.ph =ifelse(gv.Trait1>mean(gv.Trait1), 1,0)) |> mutate(rank.sum = rowSums(cbind(rank.ph,rank.gy))) |>  ungroup() 

ind.sel =geno_us |> group_by(id) |>  summarise(avg=sum(rank.sum)) 

geno_us = geno_us |>  pivot_wider(values_from = c(gv.Trait1,gv.Trait2), names_from = c(env) ) |> left_join(ind.sel, by="id") |> as.data.frame()
...
gv_df |> group_by(env) |> summarise(variance =var(gv.Trait1)) |> ggplot() + geom_col(aes(x=env, y=variance),fill="skyblue") +   theme_minimal() +
  labs(title = "CS Variance of gv.Trait1 by Environment", x = "Environment", y = "Variance") +
  ylim(0, var(gv_df_us$gv.Trait1)+0.1)

gv_df_us |> group_by(env) |> summarise(variance =var(gv.Trait1)) |> ggplot() + geom_col(aes(x=env, y=variance),fill="skyblue") +   theme_minimal() +
  labs(title = "US Variance of gv.Trait1 by Environment", x = "Environment", y = "Variance") +
  ylim(0, var(gv_df_us$gv.Trait1)+0.1)

# Create a ggplot to visualize the variance
ggplot(summarized_df, aes(x = group, y = variance)) +
  geom_bar(stat = "identity", fill = "skyblue") +
  theme_minimal() +
  labs(title = "Variance of Value by Group", x = "Group", y = "Variance")
```
